...
\begin{frame}[fragile]{Abstract}
        I propose the smart-casual beamer theme †\alert{kyasual}†!
\end{frame}
...
\begin{frame}{Introduction}
    \begin{textblock}{Purpose}
        \begin{itemize}
        \item{The conventional beamer template is not very pretty.}
        \thusitem{I design a \alert{pretty} theme.}
        \end{itemize}
    \end{textblock}
    \begin{textblock}{Proposition}
        \begin{enumerate}
            \item{\alert{pretty}.}
                \begin{itemize}
                    \item{Color scheme like sweets}
                \end{itemize}
            \item{Using in \textbf{Lightning Talk}.}
            \item{But I'd like to use in \textbf{math seminar} (≒\alert{formal}).}
            \thusitem{No guile casual, pseudo casual. i.e. \alert{kyasual}．}
            \begin{itemize}
                \annoitem{like \alert{smart-casual}}
            \end{itemize}
        \end{enumerate}
    \end{textblock}
\end{frame}
...
\begin{frame}{math sample1}
    \begin{block}{Def. 1 Sample}
        The pair $(A,R)$ of any set $A$ and the sum  $R = \bigcup_{a \in I} \to_a $ of the binary relation $\to_{\alpha}$ defined on $A$ is called Sample.
    \end{block}
...
\begin{frame}{How to Use?}
    \begin{textblock}{Basically, it is the same as an ordinary beamer.}
        \begin{itemize}
            \item{Or compare sampleslide.pdf and sampleslide.tex.}
            \item{If you wish, I will add documentation.}
                \begin{itemize}
                    \item{Please send your requests to issue or @kyawaway.}
                \end{itemize}
        \end{itemize}
    \end{textblock}
\end{frame}
...
\begin{textblock}{textblock title2}
    \begin{itemize}
        ...
        \item{Usign with \alert{itemize},}
        \okitem{become beautiful}
    \end{itemize}
\end{textblock}
            \end{lstlisting}
        \end{column}
...
\begin{simplebox}
    Not expected to be set up accordingly.
\end{simplebox}
            \end{lstlisting}
        \end{column}
        \begin{column}{0.5\textwidth}
...
\begin{frame}{Conclusion}
    \begin{textblock}{The theme finished up being very casual ....}
        \begin{itemize}
            \okitem{but I'm satisfied because it looks \alert{good}:)}
        \end{itemize}
    \end{textblock}
\end{frame}
